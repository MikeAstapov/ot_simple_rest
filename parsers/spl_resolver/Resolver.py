...
class Resolver:
    ...
    def create_subsearch(self, match_object):
        ...
        subsearch_query = re.sub(self.read_pattern, self.create_read_graph, subsearch_query)
        self.subsearches['subsearch_%s' % subsearch_uuid] = subsearch_query
        return match_object.group(0).replace('[%s]' % match_object.group(1), 'subsearch=subsearch_%s' % subsearch_uuid)
    ...
    def resolve(self, spl):
        ...
        spl = re.sub(self.read_pattern, self.create_read_graph, spl)
...
if __name__ == '__main__':
    ...
    spl3 = """search index=asd  \\[asd\\]| table 3"""
