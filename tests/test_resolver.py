...
class TestResolver(unittest.TestCase):
    ...
    def test_filter_escaped_quotes(self):
        ...
        target = {'search': ('search index=main* sourcetype!=alert_metadata| fields - _raw| dedup full_id | search alert="pprb_*" status!="*resolved" status!="suppressed" app="*" urgency="*"| stats count(alert) by alert |  where like(alert,"pprb_appcore_namedcounter%")| stats count', '| read {"main": {"query": "!(sourcetype=\\"alert_metadata\\")", "tws": 0, "twf": 0}, "main1": {"query": "!(sourcetype=\\"alert_metadata\\")", "tws": 0, "twf": 0}, "main2": {"query": "!(sourcetype=\\"alert_metadata\\")", "tws": 0, "twf": 0}}| fields - _raw| dedup full_id | filter {"query": "(alert rlike \'pprb_.*\') AND !(status rlike \'.*resolved\') AND !(status=\\"suppressed\\") AND (app rlike \'.*\') AND (urgency rlike \'.*\')"}| stats count(alert) by alert |  where like(alert,"pprb_appcore_namedcounter%")| stats count'), 'subsearches': {}}
        result = self.resolver.resolve(spl)
        print('result', result)
        print('target', target)
        self.assertDictEqual(target, result)
    ...
    def test_inputlookup_filter_with_append(self):
        ...
        target = {'search': ('| otinputlookup test.csv append=t where field1=1 OR (field2>2 AND field3<30) OR field4', '| otinputlookup test.csv append=t where {"query": "field1=\\"1\\" OR (field2>2 AND field3<30) OR (_raw like \'%field4%\')"}'), 'subsearches': {}}
        result = self.resolver.resolve(spl)
        print('result', result)
        print('target', target)
        self.assertDictEqual(result, target)
